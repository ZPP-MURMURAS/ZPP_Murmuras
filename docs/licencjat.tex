...
Sometimes the extraction model finds multiple entities with the same label. We have tested multiple policies on handling this. Our primary choice leverages the fact that the BERT model returns probabilities alongside NER tags. We simply take the entity with the greatest value, breaking ties in favor of earlier occurrence - we will refer to pipeline equipped with this algorithm as BERT-top\_score. Additionally, we have tested concatenating all the entities with matching label (BERT-concat) and taking the first entity with given label in the text (BERT-first).
