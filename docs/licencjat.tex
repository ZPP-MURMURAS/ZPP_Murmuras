...
\hline
\textbf{id} & \textbf{time} & \textbf{i} & \textbf{view\_depth} & \textbf{text} \\
\hline
...
\hline
\textbf{description} & \textbf{seen\_timestamp} & \textbf{is\_visible} & \textbf{x\_1} & \textbf{y\_1} & \textbf{x\_2} & \textbf{y\_2} \\
\hline
...
\hline
\textbf{id} & \textbf{time} & \textbf{coupon\_i} & \textbf{visible} \\
\hline
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{discount\_details} & \textbf{validity\_text} & \textbf{activation\_text} \\
\hline
...
\begin{tabular}{|l|l|l|l|l|l|l|}
\hline
\textbf{content\_proper} & \textbf{product\_text} & \textbf{extra} & \textbf{discount\_text} \\
\hline
"[Düfte]" & Düfte & 15\% & Düfte \\
\hline
\end{tabular}
...
\begin{tabular}{|l|l|l|l|l|l|l|}
\hline
\textbf{product} & \textbf{limitation} & \textbf{validity\_date} \\
\hline
  &  & 2024-03-31 \\
\hline
\end{tabular}
...
\subsubsection{Llama datasets}
As observed in the results presented in Table \ref{tab:llama_token_processing}, JSON-encoded content is processed more efficiently than plain text format in terms of token processing speed and total token count. The higher throughput for JSON-encoded content (271.32 tokens/s compared to 169.66 tokens/s for plain text) indicates that the structure inherent in JSON inputs allows for more efficient tokenization and processing by the model. This likely stems from the consistency in token boundaries within the structured format, which reduces the computational complexity typically associated with free-form plain text inputs.

Moreover, the total number of tokens processed during the same measurement duration (384 seconds) is higher for JSON, suggesting that JSON-encoded content either contains more tokens on average or has a higher token density. This result highlights that, although JSON inputs may include additional syntactic elements such as field names and brackets, these elements contribute to a greater number of tokens, potentially making the content richer in terms of token representation.

The higher tokens per timestamp (289.41 for JSON vs. 180.98 for plain text) further emphasizes the improved efficiency of JSON-encoded content. This indicates that the model can process structured data at a faster rate, likely due to the more predictable and consistent token structure that JSON provides.

\begin{table}[h!]
\centering
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{Plain Text Format} & \textbf{JSON-Encoded Content} \\
\midrule
Incoming tokens/s         & 169.66   & 271.32 \\
Total tokens              & 65,151   & 104,188 \\
Measurement duration [s]  & 384      & 384 \\
Tokens per timestamp      & 180.98   & 289.41 \\
\bottomrule
\end{tabular}
\caption{Token processing performance for plain text vs. JSON-encoded content using a LLaMA model}
\label{tab:llama_token_processing}
\end{table}
