...
This experiment showcased that simple representation of the data, without any class balancing produces best results. This was the reason why we decided to abandon JSON format and Curriculum learning. Additionally, later experiments were performed with 10 epochhs instead of 30, because with more epochs, plain model loss function values were starting to rise. On plots (\ref{fig:elb}, \ref{fig:epb}, \ref{fig:erb}), models marked as "curr" were trained with Curriculum algorithm.

For per-app fine-tuning, models generally don't work well on the data that hey didn't see. An exception here is the Lidl data, after which a model reached over 60 percent recall (\ref{fig:ers}).

With both incremental fine-tunings, while at first, DM-trained model has great difficulties with classification, further fine-tunings greatly improve its recall, which suggests that it should be easy to extend model's functionality just by simple fine-tuning (\ref{eras}, \ref{erag}).

\section{Extraction pass fine-tuning results}
With general fine-tuning, extraction pass smoothly learned. Main point of concern may be the loss, raising in pair with the recall and precision, which may suggest slight overfitting (\ref{fig:e_elg}, \ref{fig:e_epg}, \ref{fig:e_erg}).

Single-app experiment yielded similar results to the selection pass one; Lidl data seems to be the most similar to other datasets (\ref{e_ers}).

For both types of incremewntal experiment, results were also very similar to the selection pass. However, here Lidl fine-tuning improved the model performance even more, which made further fine-tuning less relevant. This shows that with right dataset, less data may be needed to expand model's abilities (\ref{e_eras}, \ref{e_erag}).



\subsection{General fine-tuning}

\subsection{Per-app fine-tuning}

\subsection{Incremental separate fine-tuning}

\subsection{Incremental combined fine-tuning}
...
\begin{itemize}
    ...
    \item \textbf{Performance Outcomes} \ref{fig:elb}, \ref{fig:epb}, \ref{fig:erb}:
    \begin{itemize}
...
The comparative results are presented in Figures~\ref{fig:elb}, \ref{fig:epb}, and~\ref{fig:erb}.
\begin{center}
   \begin{listing}
        \begin{minted}[frame=single,
                       framesep=3mm,
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{evallossbert.png}
    \caption{Eval/loss statistics of selection pass during general training}
    \label{fig:elsg}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{evalprecbert.png}
    \caption{Eval/precision statistics of selection pass during general training}
    \label{fig:epsg}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{evalrecbert.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{ers.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{eras.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{erag.png}
