...
When training without the task description, the evaluation loss plots are looking similar regardless of increment size (see plots~\ref{fig:llama-inc-15-eval}~\ref{fig:llama-inc-50-eval}~\ref{fig:llama-inc-50-eval}). The evaluation loss is at first around 0.5, then it jumps to around 0.85, finally, after the last dataset substitution, it ends around 0.9. These visualizations allow us to suggest the hypothesis that the model \emph{forgets} the overfitted dataset-specific knowledge as it no longer sees the samples from this dataset. The test loss jumps to the value above 1.4 after the first dataset swap and then drops to the value around 1.15 (see plots~\ref{fig:llama-inc-15-test}~\ref{fig:llama-inc-50-test}~\ref{fig:llama-inc-100-test}). This suggests that this methodology might be considered as interesting replacement for standard fine-tuning.

Addition of the task description further decreases the test loss, to the values below 1.1 in the later epochs for increment size equal to 10 (see~\ref{fig:llama-inc-10-test-w}), and reached even 1.05 in case of bigger increment sizes (see~\ref{fig:llama-inc-50-test-w} and~\ref{fig:llama-inc-100-test-w}).
\section{Experiments}
, point three and four. We have explored different sizes of datasets introduced alongside the training.
