...
\chapter{An alternative: fine-tuned Llama model}
Apart from the aforementioned BERT-based proposal, we have prepared a fine-tuned version of Llama\cite{meta-llama} model to measure its performance in the studied task as a scrapping tool.

We have chosen the \emph{meta/llama-3.2-1b}\cite{llama-32-intro} model as the base for the fine-tuning process. Other families of models were considered by us, but they were rejected in favor of the Llamas thanks to either their lack of support in Unsloth\cite{unsloth-model-support} and framework provided by Hugging Face or due to the missalignment of the models sizes and performances, like \emph{Gemma 2}\cite{llama-32-intro}. The following sections describe the setup, the fine-tuning and the results of performed experiments.
\section{Task setup}
We assumed that the model is expected to take the content of the text fields displayed to the user on the screen and return a list of coupons in JSON format described in \ref{sec:coupon_model}. Additionally, we have explored the model performance when the prompt was enhanced with the task description. However, no significant gain was achieved by us this way. The full description of the prompt format is given in \ref{llamaDsDesc}.
\section{Fine-Tuning}
The fine-tuning was performed on the Modal\cite{modal} platform using the Nvidia H100 GPU as a hardware. The computation was further accelerated with the help of Unsloth\cite{unsloth} tool which enhanced it with LoRA\cite{lhu2021loralowrankadaptationlargeora}, the technique that allows making weights updates with fewer number of parameters by producing the weight update matrix from product of two low dimensional matrices. In our case, we have reduced the second dimension of these to r=16. We used cross-entropy loss\cite{mao2023crossentropylossfunctionstheoretical} and AdamW optimizer adapted for 8-bit cached values\cite{hf-bnb}. Finally, we have applied the weight decay and used linear learning rate scheduler. The split between training and evaluation samples in the datasets was done with proportions 80:20.

\section{Results}
We have performed several fine-tunings with various setups. In the rest of this section the baseline experiments results is discussed, while more advanced experiments with dynamic changes to the dataset are described in the following section. We will touch only cross-entropy loss evaluated during training here as the models' performance on our benchmark is described in [ref to benchmark results]. The detailed plots for fine-tuning results are available in \ref{AppH}.

The baseline fine-tuning was performed on the datasets from four apps: REWE, ROSSMANN, LIDL and DM and on each single app separately. During the training, the test loss was computed on the datasets from PENNY and EDEKA applications.

The loss suggests that the model tries to generalize to evaluation part of the dataset in the first few epochs, reaching the loss about 0.7 \ref{fig:llama-wth-loss}. Further epochs result in overfitting. In general, model fails to generalize to the test dataset, with the loss below 1.5 after the first epoch of the training and stable growth afterwards.

Adding task description to the model prompt decreased the test loss after the first epoch to the value below 1.25, but did not prevent the loss growth in next epochs \ref{fig:llama-w-loss}.

Fine-tunings on data from single datasets resulted in drastically higher test loss in case of not providing the instruction to the model \ref{fig:llama-wth-appwise}. With instruction, the performance on the test set matched the previous experiment \ref{fig:llama-w-appwise}.
...
\textit{Llama 3.2 introduced by Meta}
\url{https://ai.meta.com/blog/llama-3-2-connect-2024-vision-edge-mobile-devices/}
[Accessed 04.05.2025]
