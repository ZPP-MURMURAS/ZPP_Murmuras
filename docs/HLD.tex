...
The widespread adoption of BERT~\cite{BERT_hf} has led to numerous architectural variants. Beyond the larger BERT-Large (around 300M parameters) and multilingual adaptations, efforts have focused on enhancing performance (e.g., RoBERTa~\cite{RoBERTa}, which outperforms vanilla BERT through extended pretraining) and improving efficiency (e.g., ALBERT~\cite{ALBERT}, which reduces parameters to  around 12M~\cite{ALBERT_hf}, and DistilBERT~\cite{DISTILBERT}, which [TODO: YYYYYYYYYYYYY"]).

After evaluating these variants, we selected the BERT-base multilingual model (179M parameters~\cite{BERT_multiling}). This decision was driven by ALBERT’s significant performance drop~\cite{BERT_comp}, RoBERTa’s lack of multilingual support~\cite{FBAIHF}, and [TODO: DistilBERT consideration], making BERT-base the optimal balance of capability and versatility for our task.

\section{ModernBERT}
During our research, ModernBERT~\cite{ModernBERTPaper} was released, offering advancements such as extended context length (8,192 tokens), faster inference, and superior performance on benchmark tasks. Despite these benefits—and its comparable size (150M parameters~\cite{ModernBERThf})—we retained BERT-base due to ModernBERT’s lack of multilingual support, which was critical for our use case.
